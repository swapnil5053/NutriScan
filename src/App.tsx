...
import { MealHistory, Meal } from './components/MealHistory';
import { analyzeFoodV2 } from './services/api';
...
export default function App() {
  ...
  const handleSaveMeal = (result: GeminiAnalysisResult) => {
    const newMeal: Meal = {
      ...
      <div className="relative z-10 flex flex-col min-h-screen w-full">
        ...
        <section className="min-h-[calc(100vh-64px)] flex items-center justify-center py-12 lg:py-0 relative">
          <div className="w-full max-w-[1000px] mx-auto px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 items-center z-10">
            ...
            >
              <div 
                className="border border-[var(--border-subtle)] bg-[var(--surface-card)] bg-opacity-30 backdrop-blur-xl rounded-[24px] p-2 aspect-[3/4] flex flex-col relative group cursor-pointer"
              >
                <ImageUpload isLoading={isLoading} onImageSelect={handleImageSelect} className="rounded-[16px] overflow-hidden bg-transparent">
                  {imagePreview && (
                    <div className="relative w-full h-full rounded-[16px] overflow-hidden bg-[var(--surface-section)]">
                       ...
                       {/* Overlay on hover or when done loading to easily replace */}
                       {!isLoading && (
                         <div className="absolute top-4 left-0 right-0 flex justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20">
                            <button 
                              className="bg-[var(--surface-card)] bg-opacity-90 backdrop-blur text-[var(--text-primary)] border border-[var(--border-subtle)] px-5 py-2 rounded-full text-caption font-medium shadow-sm pointer-events-none"
                            >
                               Replace Image
                            </button>
                         </div>
                 ...
                 {/* Analyze Button floated to the right */}
